...
invoke
	= begins:invokeBracketLeft SPACE_CHARACTER_OR_NEWLINE* inside:expressionitems SPACE_CHARACTER_OR_NEWLINE* ends:invokeBracketRight { 
...
blockSectionPart = everlasting / indentedVerbatimOperate / indentedOperate / list / paragraph
...
indentedVerbatimOperate = leader:invoke '|' NEWLINE_INDENT_ADD rear:indentedVerbatimLines INDENT_REMOVE {
...
list = head:listitem rear:(TEXT_IN_SEGMENT_LINEBREAK listitem)* {
...
listitem = "-" SPACES? content:line { return [new Reference('.li'), content] }
...
NEWLINE_INDENT_SAME_OR_MORE
	= LINE_BREAK SPACES? NEWLINE_INDENT_SAME_OR_MORE
	/ LINE_BREAK INDENT_SAME_OR_MORE

INDENT_CLEAR_ON  = { textIndentStack.push(textIndent); textIndent = ""; console.log('C', textIndentStack.concat(textIndent)) }
INDENT_CLEAR_OFF = { textIndent = textIndentStack.pop(); console.log('U', textIndentStack.concat(textIndent)) }
INDENT_ADD = spaces:SPACES & { return spaces.length > textIndent.length && spaces.slice(0, textIndent.length) === textIndent }
                  { textIndentStack.push(textIndent); textIndent = spaces; console.log('A', textIndentStack.concat(textIndent))  }
INDENT_REMOVE = { textIndent = textIndentStack.pop();  console.log('R', textIndentStack.concat(textIndent)) }
