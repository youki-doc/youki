...
quasiquote
	= '`' it:(invoke/verbatim/textblock/identifier) { return [new Reference('quasiquote'), it] }
unquote
	= ',' it:(invoke/verbatim/textblock/identifier) { return [new Reference('unquote'), it] }
attribute
	= ":" it:identifier { return [new Reference('.attribute'), it.id] }
literal
	= numberliteral
	/ stringliteral
...
verbatimTerminator
	= "|" terminator:verbatimEqualSequence "}" & { return terminator === storedVerbatimTerminator } { return }
