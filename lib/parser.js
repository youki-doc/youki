...
function lexLine(s){
	...
	walk(INLINE, s,
		function(m, marco, _2, verbatim, bracket, _5, sourcely, quickmark, newline){
			if(marco) {
				if(marco != '\,')
				...
				push({type: LB});
				push({type: LIT, text: verbatim, raw: true});
				push({type: RB});
			} else if(_5) {
				push({type: NOMEN, text: '.inline_code'});
				push({type: LB});
				push({type: LIT, text: sourcely, raw: true})
				push({type: RB});
				push({type: FIN})
			} else if(quickmark) {
				push({type: QM, text: quickmark})
			} else if(newline) {
				if(/:/.test(m)) {
...
function parse(s){
	...
	function line(qm_text) {
		...
		while(token.type && token.type !== RB && token.type !== NL && (!qm_text || token.type !== QM && token.text !== qm_text)) {
			if(token.type === LIT){
				buff.push([ref(token.raw ? '.raw' : '.lit'), token.text]);
				move();
